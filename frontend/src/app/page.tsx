                        ...
                        <div className="bg-white/80 dark:bg-[#0c0c0e] backdrop-blur-2xl border border-blue-500/30 rounded-[32px] p-5 sm:p-8 shadow-2xl relative overflow-hidden group">
                          ...
                          <div className="relative z-10">
                            ...
                            <div className="prose prose-sm dark:prose-invert max-w-none bg-neutral-50 dark:bg-white/5 p-6 rounded-[24px] border border-black/5 dark:border-white/5">
                              <ReactMarkdown
                                  components={{
                                      h1: ({node, ...props}) => <h1 className="text-2xl font-black mb-4" {...props} />,
                                      h2: ({node, ...props}) => <h2 className="text-xl font-black mt-6 mb-3 text-blue-600" {...props} />,
                                      blockquote: ({node, ...props}) => <blockquote className="border-l-4 border-blue-500/20 bg-blue-500/5 p-4 rounded-r-2xl italic" {...props} />
                                  }}
                              >
                                {analysis.ultimateSynthesis || analysis.consensus}
                              </ReactMarkdown>
                            </div>
                            
                            <div className="mt-8 pt-6 border-t border-black/5 dark:border-white/5 flex flex-wrap items-center justify-between gap-4">
                               <div className="flex items-center gap-3">
                                  <div className="px-4 py-2 rounded-xl bg-blue-500/10 text-blue-600 text-[10px] font-black uppercase tracking-widest border border-blue-500/20">Generated by Master Fusion</div>
                                  <div className="px-4 py-2 rounded-xl bg-emerald-500/10 text-emerald-600 text-[10px] font-black uppercase tracking-widest border border-emerald-500/20">Verified Accuracy</div>
                               </div>
                               <button 
                                  onClick={() => {
                                      ...
                                      const a = document.createElement("a");
                                      a.href = url;
                                      a.download = `fusion-synthesis-${Date.now()}.md`;
                                      a.click();
                                  }}
                                  className="flex items-center gap-2 px-6 py-2.5 bg-neutral-900 dark:bg-white text-white dark:text-black rounded-xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-all"
                               >
