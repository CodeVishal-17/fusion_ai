...
import { Upload, FileText, Trash2, Database, Search, CheckCircle2, Shield } from 'lucide-react';
...
export default function KnowledgeSection() {
    ...
    useEffect(() => {
        // Mock fetch files - in real app would fetch from /api/knowledge/list
        setFiles([
            { id: '1', fileName: 'Company_Strategy_2026.pdf', fileType: 'application/pdf', createdAt: new Date() },
            { id: '2', fileName: 'Product_Specs_v3.docx', fileType: 'application/docx', createdAt: new Date() }
        ]);
    }, []);
    ...
    const handleUpload = async (e: any) => {
        ...
        try {
            ...
            if (data.fileName) {
                setFiles(prev => [...prev, { id: Math.random().toString(), fileName: data.fileName, fileType: file.type, createdAt: new Date() }]);
            }
        } catch (err) {
                ...
                <div className="md:col-span-2 space-y-6">
                    ...
                    <div className="space-y-3">
                        {files.filter(f => f.fileName.toLowerCase().includes(searchQuery.toLowerCase())).map((file) => (
                            <div key={file.id} className="flex items-center justify-between p-4 bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl group hover:shadow-lg transition-all">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-[9px] text-neutral-400 font-bold uppercase tracking-tighter">{file.fileType.split('/')[1]}</span>
